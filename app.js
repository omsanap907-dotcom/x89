...
function render(){let a=[...tools];if(query)a=a.map(t=>({...t,_score:score(t,query)})).filter(t=>t._score>0).sort((x,y)=>y._score-x._score);else a.sort((x,y)=>x.name.localeCompare(y.name));document.querySelector("#title").textContent=query?"Search results":"All tools";document.querySelector("#count").textContent=a.length+" tool"+(a.length===1?"":"s");document.querySelector("#hint").textContent=query?"Related matches":"Pick one";document.querySelector("#active").innerHTML=query?'<button class="active-btn" id="qx">“'+esc(query)+'” ×</button>':"";document.querySelector("#qx")?.addEventListener("click",()=>{query="";document.querySelector("#search").value="";render()});document.querySelector("#results").innerHTML=a.map(t=>'<article class="tool"><div class="tooltop"><a href="'+esc(t.url)+'" target="_blank" rel="noopener">'+esc(t.name)+'</a><a class="open" href="'+esc(t.url)+'" target="_blank" rel="noopener">OPEN ↗</a></div><p>'+esc(t.description)+'</p><div class="tags">'+t.categories.slice(0,3).map(c=>'<span class="tag">'+esc(c)+'</span>').join("")+'</div></article>').join("");document.querySelector("#empty").hidden=a.length>0}
